...
def get_subtitle_save_path(video_path: str, lang_code: str = "zh", plex_format: bool = True, file_ext: str = "srt") -> str:
    """
    获取字幕保存路径
    ...
    Args:
        ...
        file_ext: 字幕文件扩展名（不含点号），如 "srt", "ass", "ssa", "vtt" 等

    PLEX 字幕命名格式:
    - 电影: Movie.Name.2024.1080p.zh.srt 或 Movie.Name.2024.1080p.zh-cn.ass
    - 剧集: Show.Name.S01E01.1080p.zh.srt 或 Show.Name.S01E01.1080p.zh-cn.ass
    """
    video_dir = os.path.dirname(video_path)
    video_name = os.path.splitext(os.path.basename(video_path))[0]

    # 标准化扩展名（去掉前导点号）
    ext = file_ext.lstrip(".").lower() if file_ext else "srt"
    ...
    if plex_format:
        ...
        return os.path.join(video_dir, f"{video_name}.{plex_lang}.{ext}")
    else:
        return os.path.join(video_dir, f"{video_name}.{lang_code}.{ext}")


def get_plex_subtitle_filename(video_filename: str, lang_code: str = "zh", file_ext: str = "srt") -> str:
    """
    获取 PLEX 格式的字幕文件名
    ...
    return f"{video_name}.{plex_lang}.{ext}"
