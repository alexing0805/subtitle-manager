...
class SubtitleManager:
    ...
    def _normalize_subtitle_encoding(self, subtitle_path: str) -> Optional[str]:
        """
        将字幕文件编码统一为 UTF-8，保留原始格式（ASS/SRT/VTT/SMI 等不做格式转换）。
        返回归一化后的文本内容，失败返回 None。
        """
        try:
            with open(subtitle_path, "rb") as f:
                raw_content = f.read()
            ...
            logger.error(f"字幕编码归一化失败 {subtitle_path}: {e}")
            return None
    ...
    def _normalize_non_text_subtitle(self, video_path: str, subtitle_path: str) -> Dict[str, Any]:
        ...
        suffix = os.path.splitext(subtitle_path)[1].lower().lstrip(".")
        target_path = get_subtitle_save_path(
            video_path, lang_code="zh-cn",
        ...
        return {
            "path": final_path,
            "converted": False,
            "warning": f"当前字幕格式 .{suffix} 无法转换为文本格式，已按 Plex 命名保留原格式",
        }
    ...
    def _normalize_downloaded_subtitle(self, video_path: str, subtitle_path: str) -> Dict[str, Any]:
        ...
        标准化下载的字幕文件：
        - 统一编码为 UTF-8（文本字幕）
        - 保留原始字幕格式（SRT 保持 SRT，ASS 保持 ASS，不做格式转换）
        - 生成 Plex 友好命名
        - 图形字幕（SUP/IDX/SUB）保持原格式仅重命名
        """
        suffix = os.path.splitext(subtitle_path)[1].lower()

        # 1. 处理 7z 压缩包：解压后继续处理内部字幕
        if suffix == ".7z":
        ...
        # 2. 图形字幕：仅做 Plex 命名，不做格式转换
        if suffix in {".sup", ".idx", ".sub"}:
            return self._normalize_non_text_subtitle(video_path, subtitle_path)

        # 3. 文本字幕（SRT/ASS/SSA/VTT/SMI/SAMI）：编码归一化为 UTF-8，保留原始格式
        TEXT_SUBTITLE_EXTENSIONS = {".srt", ".ass", ".ssa", ".vtt", ".smi", ".sami"}
        if suffix in TEXT_SUBTITLE_EXTENSIONS:
            normalized_text = self._normalize_subtitle_encoding(subtitle_path)
            if normalized_text is None:
                logger.warning(f"字幕编码归一化失败，保留原文件: {subtitle_path}")
                target_path = get_subtitle_save_path(
                    video_path, lang_code="zh-cn",
                    plex_format=settings.PLEX_NAMING_FORMAT,
                ...
                final_path = self._replace_subtitle_file(subtitle_path, target_path)
                return {"path": final_path, "converted": False, "warning": "字幕编码归一化失败，已保留原格式"}
            ...
            if settings.BACKUP_EXISTING_SUBTITLE:
                self._backup_existing_file(target_path)
            elif os.path.exists(target_path) and os.path.normcase(subtitle_path) != os.path.normcase(target_path):
                os.remove(target_path)

            with open(target_path, "w", encoding="utf-8", newline="\n") as file_obj:
                file_obj.write(normalized_text)

            if os.path.exists(subtitle_path) and os.path.normcase(subtitle_path) != os.path.normcase(target_path):
                os.remove(subtitle_path)

            logger.info(f"字幕已归一化为 UTF-8（保留 {suffix} 格式）: {target_path}")
            return {"path": target_path, "converted": False}

        # 4. 其他未知格式：仅做 Plex 命名
        target_path = get_subtitle_save_path(
            video_path, lang_code="zh-cn",
            plex_format=settings.PLEX_NAMING_FORMAT,
            file_ext=suffix.lstrip(".") or "srt",
        )
        final_path = self._replace_subtitle_file(subtitle_path, target_path)
    ...
    async def upload_single_subtitle(self, episode_id: str, subtitle_file):
        ...
        subtitle_base = get_subtitle_save_path(
            video_path, lang_code="zh-cn", plex_format=settings.PLEX_NAMING_FORMAT,
            file_ext=suffix.lstrip("."),
        )
        temp_path = os.path.splitext(subtitle_base)[0] + f".upload{suffix}"
